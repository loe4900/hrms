$(function () {
    ...
    //1.第一次加载页面，需要首页数据
    $.get(basePath + '/dept/list', {dept: null, pageIndex: 1, pageSize: 4}, function (data) {
        dataHandler(data);
    }, 'json');
});
...
function listHandler(list) {
    ...
    for (var index = 0; index < list.length; index++) {
        ...
        // 组行和列并附加到tbody
        $('tbody').append(tr.append(th).append(deptId).append(name).append(remark).append(operation));
    }
}
...
function prePage(pageIndex, pageSize) {
    ...
    if (pageIndex - 1 >= 1) {
        $.get(basePath + '/dept/list', {name : name, remark:remark, pageIndex: pageIndex - 1, pageSize: pageSize}, function (data) {
            dataHandler(data);
        }, 'json');
    }
}
...
function currPage(that, pageSize) {
    var name = $('#name').val();
    var remark = $('#remark').val();
    $.get(basePath + '/dept/list', {name : name, remark:remark, pageIndex: that.text, pageSize: pageSize}, function (data) {
        dataHandler(data);
    }, 'json');
}
...
function deleteItem(that) {
    ...
    // 3.按钮事件
    $('#deptModalCenter .modal-footer .btn-primary').unbind().click(function () {
        // 1.关闭之前的模态框
        ...
        $.getJSON(basePath + '/dept/del/' + deptId, function (data) {
            alert(data.message);
            // 3-1.刷新列表
            $.getJSON(basePath + '/dept/list', {dept: null, pageIndex: 1, pageSize: 4}, function (data) {
                dataHandler(data);
            });
        });
    });
...
//1.获取查询关键字
    ...
    var name = $('#name').val();
    //2.请求服务器
    $.getJSON(basePath + '/dept/list', {name : name, remark:remark, pageIndex: 1, pageSize: 4}, function (data) {
        dataHandler(data);
    });
...
function initModal(title, body, btn1, btn2) {
    // 标题
    $('#deptModalCenter .modal-title').text(title);
    // 内容
    $('#deptModalCenter .modal-body').text(body);
    // 按钮
    if (btn1)
        $('#deptModalCenter .modal-footer .btn-secondary').text(btn1);
    if (btn2)
        $('#deptModalCenter .modal-footer .btn-primary').text(btn2);

    if (!btn1 && !btn2)
        $('#deptModalCenter .modal-footer').hide();
    else
        $('#deptModalCenter .modal-footer').show();
}