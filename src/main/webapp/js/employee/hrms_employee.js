$(function () {
    ...
    //1.第一次加载页面，需要首页数据
    $.get(basePath + '/employee/list', function (data) {
        dataHandler(data);
    }, 'json');
});
...
function listHandler(list) {
    ...
    for (var index = 0; index < list.length; index++) {
        ...
        var id = $('<td>' + list[index].id + '</td>');
        var deptId = $('<td>' + list[index].dept.id + '</td>');
        var jobId = $('<td>' + list[index].job.id + '</td>');
        var name = $('<td>' + list[index].name + '</td>');
        var cardId = $('<td>' + list[index].cardId + '</td>');
        var address = $('<td>' + list[index].address + '</td>');
        var postcode = $('<td>' + list[index].postcode + '</td>');
        var tel = $('<td>' + list[index].tel + '</td>');
        var phone = $('<td>' + list[index].phone + '</td>');
        var qqNum = $('<td>' + list[index].qqNum + '</td>');
        var email = $('<td>' + list[index].email + '</td>');
        var set = $('<td>' + list[index].sex + '</td>');
        ...
        var speciality = $('<td>' + list[index].speciality + '</td>');
        var hobby = $('<td>' + list[index].hobby + '</td>');
        ...
        // 组行和列并附加到tbody
        $('tbody').append(tr.append(ck).append(th).append(id).append(deptId).append(jobId).append(name).append(cardId).append(address)
            .append(postcode).append(tel).append(phone).append(qqNum).append(email).append(set).append(party).append(birthday)
            .append(race).append(education).append(speciality).append(hobby).append(remark).append(createDate).append(operation));
    }
}
...
function prePage(pageIndex, pageSize) {
    ...
    if (pageIndex - 1 >= 1) {
        $.get(basePath + '/employee/list', {
            name: name,
            createDate: createDate,
            phone: phone,
            qqNum:qqNum,pageIndex: pageIndex - 1, pageSize: pageSize
        }, function (data) {
            dataHandler(data);
        }, 'json');
    }
...
function getIdOfRow(that) {
    return $(that).parent().siblings().eq(2).text();
}
...
function initModal(title, body, btn1, btn2) {
    // 标题
    $('#empModalCenter .modal-title').text(title);
    // 内容
    $('#empModalCenter .modal-body').text(body);
    // 按钮
    if (btn1)
        $('#empModalCenter .modal-footer .btn-secondary').text(btn1);
    if (btn2)
        $('#empModalCenter .modal-footer .btn-primary').text(btn2);

    if (!btn1 && !btn2)
        $('#empModalCenter .modal-footer').hide();
    else
        $('#empModalCenter .modal-footer').show();
}
/**
 * 查看记录
 * @param that
...
function lookItem(that) {
    // 1.获得查看行id
    var employeeId = getIdOfRow(that);
    // 2.加载查看页面
    // 2-1.查看页面是请求服务器得到的
    $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/employee/look/" + employeeId + "' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
}
...
function updateItem(that) {
    ...
    // 2.加载更新页面
    // 2-1.更新页面是请求服务器得到的
    $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/employee/update?id=" + employeeId + "' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
}
...
function deleteItem(that) {
    ...
    // 3.按钮事件
    $('#empModalCenter .modal-footer .btn-primary').unbind().click(function () {
        // 1.关闭之前的模态框
        $('#empModalCenter').modal('hide');
        // 2.获得删除行的noticeId
        var employeeId = getIdOfRow(that);
        // 3.请求服务器删除该记录
        $.getJSON(basePath + '/employee/del/' + employeeId, function (data) {
            alert(data.message);
            // 3-1.刷新列表
            $.getJSON(basePath + '/employee/list', function (data) {
                dataHandler(data);
            });
        });
    });
...
function batchDeleteItems() {
    ...
    $('#empModalCenter .modal-footer .btn-primary').unbind().click(function () {
        ...
        // 3.请求服务器删除该记录
        $.getJSON(basePath + '/employee/batchDelete', {ids: noticeIds}, function (data) {
            alert(data.message);
            // 3-1.刷新列表
            $.getJSON(basePath + '/employee/list', function (data) {
                dataHandler(data);
 ...
 */
function addItem() {
    $(parent.document).find('.page-wrapper').html("<iframe src='" + basePath + "/employee/add' width='100%' height='100%' frameborder='0' name='_blank' id='_blank'></iframe>");
}
...
function query() {
    ...
    // 2.请求服务器
    $.getJSON(basePath + '/employee/list', {name: name, createDate: createDate, phone: phone,email:email}, function (data) {
        dataHandler(data);
    });
}